...
public class GentableController extends BaseController {
    ...
    public Map<String, Object> tableListPage(@RequestBody PageData pd, HttpServletResponse response) throws Exception {
        //处理分页
        if(Verify.verifyIsNull(pd.get("page"))){
            pd.put("page",1);
        }
        if(Verify.verifyIsNull(pd.get("limit"))){
            pd.put("limit",10);
        }
        pd.put("start_page",(Integer.parseInt(pd.get("page").toString())-1)*Integer.parseInt(pd.get("limit").toString()));
        ...
        dataTable.put("list",list);
        PageData p = new PageData();
        p.put("total",num);
        p.put("page",pd.get("page"));
        p.put("size",pd.get("limit"));
        dataTable.put("pagination",p);
        return dataTable;
    }
     ...
     */
    @GetMapping("/listPage")
    public Map<String, Object> listPage(QueryRequest queryRequest, Gentable gentable) {
...
//        String userParams = SecurityContextHolder.getContext().getAuthentication().getName();
    ...
    public void gencode(@RequestParam String id, HttpServletResponse response) throws Exception  {
        ...
        List<Genfield> fieldList = genfieldService.findFieldList(pd);
        if(gentable.getTemp_type()==0){//单表
        }else if(gentable.getTemp_type()==1){//树表
        ...
        //生成columns.tsx
        fprint("gencode/TemplateColumns.tsx.ftl", root, gen_path+gentable.getBus_name()+File.separator+gentable.getMod_name()+File.separator+FreemarkerParaUtil.vue+gentable.getBus_name()+"/columns.tsx");
        //生成formSchemas.tsx
        fprint("gencode/TemplateFormSchemas.tsx.ftl", root, gen_path+gentable.getBus_name()+File.separator+gentable.getMod_name()+File.separator+FreemarkerParaUtil.vue+gentable.getBus_name()+"/formSchemas.tsx");
        if(gentable.getTemp_type()==1) {//树表
            //生成TREE
        ...
        //生成API
        fprint("gencode/TemplateIndex.ts.ftl", root, gen_path+gentable.getBus_name()+File.separator+gentable.getMod_name()+File.separator+FreemarkerParaUtil.vue+gentable.getBus_name()+"/index.ts");
        //生成model.d.ts
        fprint("gencode/TemplateModel.d.ts.ftl", root, gen_path+gentable.getBus_name()+File.separator+gentable.getMod_name()+File.separator+FreemarkerParaUtil.vue+gentable.getBus_name()+"/model.d.ts");
