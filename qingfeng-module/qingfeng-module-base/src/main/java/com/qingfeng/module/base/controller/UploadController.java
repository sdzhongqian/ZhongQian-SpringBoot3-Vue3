...
public class UploadController extends BaseController {
     ...
     * @throws Exception
     */
    @RequestMapping(value = "/uploadRich", method = RequestMethod.POST)
    public MyResponse uploadRichText(HttpServletRequest request, HttpServletResponse response , HttpSession session, MultipartFile file) throws Exception {
        PageData pd = new PageData(request);
        ...
        //保存文件
        String savePath = ParaUtil.localName;
        String path = ParaUtil.common+"richtext/"+DateTimeUtil.getDate()+"/"+ GuidUtil.getGuid()+fileSuffix;
        File files = new File(savePath+path);
        if (!files.getParentFile().exists()){
            files.getParentFile().mkdirs();
        }
    ...
    public void uploadLocalFile(HttpServletRequest request, HttpServletResponse response , HttpSession session, MultipartFile file) throws Exception {
        ...
        pd.put("file_size",files.length());
        pd.put("show_path",ParaUtil.cloudfile+path);
        this.writeJson(response,new Json(true,200,pd,"附件上传成功"));
    }
