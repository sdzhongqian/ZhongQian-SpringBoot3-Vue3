...
import { login, getInfo, logout,verifyLogin } from '@/api/login';
  ...
  actions: {
    ...
    async afterLogin() {
      try {
        ...
        const [userResp, menuResp] = await Promise.all([getInfo(), findMenuList({ types: '0,1,3,4' })]);
        const userInfo = userResp.data;
        const menus = menuResp.data;
        const perms = userInfo.authList;
        ...
        this.avatar = userInfo.head_address;
        this.userInfo = userInfo;
        // 生成路由
        const generatorResult = await generatorDynamicRouter(menus, role);
    ...
    async verifyLogin(params){
      try {
        const { data } = await verifyLogin(params);
        return data;
      } catch (error) {
        return Promise.reject(error);
      }
